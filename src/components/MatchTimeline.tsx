...
import { Round, Player, Match } from '../logic/types';
    ...
    <div className="space-y-2.5">
      ...
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-2">
          <span className="text-base font-extrabold text-slate-800">🏸 対戦スケジュール</span>
            ...
            >
              ...
              <div className="py-1.5 space-y-1.5">
                {currentRoundData.matches.map((match, mIdx) => (
                  <div
                    key={mIdx}
                    className="rounded-xl p-2 bg-slate-50 border border-slate-100 space-y-1"
                  >
                    ...
                    {isEditing ? (
                      ...
                      <div className="flex items-center gap-1.5 text-xs">
                        {/* Team 1 Dropdowns */}
                        <div className="flex-1 space-y-1 bg-blue-50 p-1.5 rounded-lg border border-blue-200">
                          <div className="text-[10px] font-black text-blue-600 text-center">ペア A</div>
                          <select
                            value={match.team1[0]}
                            onChange={(e) =>
                              handleDraftPlayerChange(mIdx, 1, 0, e.target.value)
                            }
                            className="w-full text-sm font-bold p-1 bg-white border border-blue-300 rounded"
                          >
                            {activePlayers.map((p) => (
                        ...
                        <div className="text-xs font-black text-slate-400 px-0.5">VS</div>

                        {/* Team 2 Dropdowns */}
                        <div className="flex-1 space-y-1 bg-rose-50 p-1.5 rounded-lg border border-rose-200">
                          <div className="text-[10px] font-black text-rose-600 text-center">ペア B</div>
                          <select
                            value={match.team2[0]}
                            onChange={(e) =>
                              handleDraftPlayerChange(mIdx, 2, 0, e.target.value)
                            }
                            className="w-full text-sm font-bold p-1 bg-white border border-rose-300 rounded"
                          >
                            {activePlayers.map((p) => (
                              <option key={p.id} value={p.id}>{p.name}</option>
                            ))}
                      ...
                      /* Normal Compact Display (Bold Large Text) */
                      <div className="flex items-center justify-between px-1 py-0.5">
                        <div className="flex items-center justify-center flex-1 min-w-0 font-black gap-1.5 text-sm sm:text-base">
                          <span className="text-blue-900 truncate text-right">
                            {getPlayerName(match.team1[0])}&thinsp;・&thinsp;{getPlayerName(match.team1[1])}
                          </span>
                          <span className="text-xs font-black text-slate-400 px-1 shrink-0">
                            vs
                          </span>
                          <span className="text-rose-900 truncate text-left">
                ...
                {isEditing && (
                  <div className="flex items-center gap-1.5 shrink-0">
                    <button
                      ...
                      className="px-3 py-1 text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg text-xs font-bold"
                    >
                      キャンセル
                    </button>
                    <button
                      ...
                      className="px-3.5 py-1 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white rounded-lg text-xs font-bold shadow-2xs"
                    >
                      変更を保存
                    </button>
                  </div>
      ...
      <div className="bg-white border border-slate-200/90 rounded-2xl p-3.5 shadow-sm space-y-3 mt-3">
        ...
        <div className="flex items-center justify-between p-2.5 bg-slate-50 border border-slate-200 rounded-xl">
          ...
          <button
            ...
            className="px-3.5 py-2 bg-white border border-slate-300 hover:bg-slate-100 active:bg-slate-200 text-slate-900 text-xs font-bold rounded-lg shadow-2xs transition-colors shrink-0"
          >
            設定を変える
          </button>
        </div>
        ...
        <div className="space-y-2.5 pt-1">
          <div className="flex items-center justify-between">
            <label htmlFor="timeline-lookahead-select" className="text-slate-800 text-sm font-bold flex items-center gap-1.5">
              <span className="text-base">🔮</span> 次に生成する試合数
            </label>
            <select
